...
if command -v curl &> /dev/null; then
	...
	echo -e "$YELLOW_COLOR使用命令安装 使用命令安装:sudo apt update && sudo apt install iputils-ping curl$RES"
    exit 
fi


# 安装必须的命令
for i in  expect dos2unix jq netcat git; do
...
EOF
  ...
  if [[ $result -eq 1 ]];then
    echo -e "${RED_COLOR}文件$config_file拷贝到${control_machines[0]}@$i失败$RES"
    #变量接收分发脚本失败的函数
  ...
  echo -e "${RED_COLOR}文件$config_file不存在$RES"
  exit 
fi
...
do
   ...
   output=$(expect <<EOF #&>/dev/null
        set timeout 300
        spawn  ssh  -p${control_machines[2]} ${control_machines[0]}@$i "sudo bash /tmp/$env_file  ${klustron_info[@]} && sudo rm -f /tmp/$env_file"
        expect {
            "yes/no" { send "yes\n"; exp_continue }
            "password" {
                send -- {${control_machines[1]}}
...
EOF
 ...
 result_error=$(echo "$output"|sed 's/[^[:print:]\t]//g'|dos2unix|tr "'"  " "|xargs -n 1|egrep -vw "${control_machines[0]}@$i|s|password\:|$env_file"|awk 'NR>12')
